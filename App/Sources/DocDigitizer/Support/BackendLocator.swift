...
/// development (running from `.build/…`) and when bundled inside DocDigitizer.app.
enum BackendLocator {

    /// Directory containing `pipeline/` and `.venv/`.
    static func backendDirectory() -> URL? {
        // 1) explicit override (handy for development)
        if let p = ProcessInfo.processInfo.environment["DOCDIGITIZER_BACKEND"] {
            let u = URL(fileURLWithPath: p, isDirectory: true)
            if hasBackend(u) { return u }
        }
        // 2) bundled: DocDigitizer.app/Contents/Resources/Backend
